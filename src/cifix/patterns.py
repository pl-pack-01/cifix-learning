...
Each entry: (compiled_regex, error_type, severity, suggestion)
"""
...
# (pattern, error_type, severity, suggestion)
PatternEntry = tuple[re.Pattern, str, ErrorSeverity, str]
...
INFRA_PATTERNS: list[PatternEntry] = [
    ...
    (re.compile(r"(Could not find a version|No matching distribution|package .* not found)", re.I),
     ...
     "Package not found. Check package name, version pin, and index URL."),
    (re.compile(r"(hash mismatch|integrity check|checksum)", re.I),
     "dependency_integrity", ErrorSeverity.ERROR,
     "Package integrity check failed. Clear cache and retry."),
